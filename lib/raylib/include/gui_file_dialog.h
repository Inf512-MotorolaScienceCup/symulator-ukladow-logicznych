...
GuiFileDialogState InitGuiFileDialog(int width, int height, const char *initPath, bool active)
{
    ...
    state.position = { GetScreenWidth()/2 - state.size.x/2, GetScreenHeight()/2 - state.size.y/2 };
...
void GuiFileDialog(GuiFileDialogState *state)
{
    if (state->fileDialogActive)
    {
        ...
        state->fileDialogActive = !GuiWindowBox({ state->position.x + 0, state->position.y + 0, (float)winWidth, (float)winHeight }, "#198# Select File Dialog");

        if (GuiButton({ state->position.x + winWidth - 50, state->position.y + 35, 40, 25 }, "< ..")) // || IsKeyReleased(KEY_DPAD_Y))
        {
        ...
        if (GuiTextBox({ state->position.x + 10.0f, state->position.y + 35.0f, winWidth - 65.0f, 25.0f }, state->dirPathText, 256.0f, state->dirPathEditMode))
        {
...
# else
        state->filesListActive = GuiListViewEx({ state->position.x + 10.0f, state->position.y + 70.0f, winWidth - 20.0f, winHeight - 135.0f }, (const char**)dirFilesIcon, state->dirFilesCount, &state->itemFocused, &state->filesListScrollIndex, state->filesListActive);
# endif
        ...
        GuiLabel({ state->position.x + 10, state->position.y + winHeight - 60, 68, 25 }, "File name:");

        if (GuiTextBox({ state->position.x + 75.0f, state->position.y + winHeight - 60.0f, winWidth - 200.0f, 25.0f }, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        state->fileTypeActive = GuiComboBox({ state->position.x + 75.0f, state->position.y  + winHeight - 30.0f, winWidth - 200.0f, 25.0f }, "All files", state->fileTypeActive);
        GuiLabel({ state->position.x + 10, state->position.y + winHeight - 30, 68, 25 }, "File filter:");

        state->SelectFilePressed = GuiButton({ state->position.x + winWidth - 120, state->position.y + winHeight - 60, 110,
...
#ifdef PLATFORM_DESKTOP
        if (GuiButton({ state->position.x + winWidth - 120, state->position.y + winHeight - 30, 110, 25 }, "Quit")) state->fileDialogActive = false;
#endif
